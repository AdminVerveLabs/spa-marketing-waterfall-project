...
- [x] **Integrate pipeline with dashboard** — Dashboard-pipeline integration deployed (ADR-032, Session 50). Webhook accepts POST, run_id flows through pipeline, batch progress tracked. **Pending user action:** run `scripts/dashboard-schema.sql` in Supabase, set dashboard env vars, and test end-to-end.

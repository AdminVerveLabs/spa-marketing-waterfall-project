...
- **Created `docs/CLAUDE-CODE-BOOTSTRAP-PROMPT.md`** — self-contained CLAUDE.md-format prompt for bootstrapping a similar n8n pipeline project in a fresh Claude Code session. Encodes all 20 pitfalls as actionable rules covering: architecture blueprint, 12 n8n-specific rules, Supabase patterns, API integration playbook, batch processing pattern, report generation pattern, tracking conventions, Docker/Coolify deployment, and a 20-item pitfalls checklist.

### Session 89 — 2026-02-27 (Apollo Sync: Fix MCP mode-stripping — BUG-055 corrected)
- **Corrected BUG-055 root cause:** Sessions 87-88 MCP `updateNode` deployments replaced the entire `parameters` object with just `{ jsCode }`, stripping the `mode` field from 5 of 7 Code nodes. Upsert Account defaulted to `runOnceForAllItems` instead of `runOnceForEachItem`, processing only 1 item per batch instead of 25.
- **Deployed mode fixes** for all 5 affected nodes via MCP, always including both `jsCode` AND `mode` in the update.
- **Verified:** All 7 Code nodes have explicit `mode` in deployed workflow. 0 validation errors.
- Local snapshot updated to match deployed workflow.
